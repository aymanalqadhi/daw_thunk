...
#ifndef DAW_THUNK_IMPL_THUNK_AAPCS64_H
#define DAW_THUNK_IMPL_THUNK_AAPCS64_H
...
struct alignas(16) thunk {
  ...
  constexpr thunk() noexcept : thunk(std::make_index_sequence<ParamCount>{}) {}

  constexpr void set_data_pointer(std::uintptr_t addr) {
    set_imm_mov<0>(addr,
                   op_codes + static_cast<std::ptrdiff_t>(ParamCount) + 4);
  }

  constexpr void set_function_pointer(std::uintptr_t addr) {
    set_imm_mov<16>(addr, op_codes + static_cast<std::ptrdiff_t>(ParamCount));
  }
};
...
set_thunk_params(Thunk& th, void* user_data_pointer, void* function_pointer) {
  th->set_function_pointer(reinterpret_cast<std::uintptr_t>(function_pointer));
  th->set_data_pointer(reinterpret_cast<std::uintptr_t>(user_data_pointer));
}
...
#endif /* ifndef DAW_THUNK_IMPL_THUNK_AAPCS64_H */
