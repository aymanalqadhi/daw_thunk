...
struct Thunk<Result(Params...)> {
  ...
  DAW_ATTRIB_NOINLINE explicit Thunk(void* user_data_pointer,
                                     Result (*function_pointer)(void*,
                                                                Params...)) {
    static_assert(sizeof(Result (*)(void*, Params...)) == sizeof(void*),
                  "Unexpected function pointer size");
    void* tmp = ::VirtualAlloc(nullptr,
                               sizeof(thunk_t),
    ...
    thunk = uptr_thunk_t(reinterpret_cast<thunk_t*>(
        memcpy(tmp, &thunk_impl::default_thunk<param_count>, sizeof(thunk_t))));

    thunk_impl::set_thunk_params(thunk,
                                 user_data_pointer,
                                 reinterpret_cast<void*>(function_pointer));
    auto old_protection = DWORD();
    if (::VirtualProtect(thunk.get(),
                         sizeof(thunk_t),
                         PAGE_EXECUTE_READ,
                         &old_protection) == 0)
    {
  ...
  explicit Thunk(Func& f) : Thunk(erased_callable<Result(Params...)>(f)) {}
