...
int main() {
...

  {
    std::cout << "\n2 param\n*******\n";
    x                 = 0;
    auto const lambda = [&](int y0, int y1) { x += y0 + y1; };
    auto const th     = daw::make_thunk(lambda);
    std::cout << x << '\n';
    th(2, 1);
    std::cout << x << '\n';
  ...
  }
  {
    std::cout << "\n3 param\n*******\n";
    x                 = 0;
    auto const lambda = [&](int y0, int y1, int y2) { x += y0 + y1 + y2; };
    auto const th     = daw::make_thunk(lambda);
    std::cout << x << '\n';
    th(2, 1, 1);
    std::cout << x << '\n';
  ...
  }
  {
    ...
    x = 0;
    auto const lambda =
        [&](T y0, T y1, T y2, T y3, T y4, T y5, T y6, T y7, T y8) {
